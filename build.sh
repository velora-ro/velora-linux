...
# ── Users & passwords ─────────────────────────────────────────
# Root - no password for live session
passwd -d root

# Velora live user - creat o singura data, fara parola
useradd -m -s /bin/bash -G sudo,audio,video,cdrom,dip,floppy,plugdev,netdev velora 2>/dev/null || true
passwd -d velora
